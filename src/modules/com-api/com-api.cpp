...
IOCode Port::Open() {
  ...
  return this->hCom != nullptr ? IOCode::QUERY_SUCCESS
                               : IOCode::ERROR_INVALID_VALUE;
}
...
IOCode Port::QueryPort() {
  ...
  HANDLE hCom =
      CreateFileA(wr::stringToLpFileName(this->attr_system_name),
...
IOCode Port::Close() {
  if (this->hCom != nullptr) {
    return CloseHandle(this->hCom) ? IOCode::QUERY_SUCCESS
                                   : IOCode::ERROR_INVALID_VALUE;
  } else {
    return IOCode::ERROR_INVALID_PORT;
  }
...
int Port::GetSystemID() { return attr_system_id; }
...
PortCollection ScanPorts() {
  ...
  for (int i = 1; i < 256; i++) {
    ...
    port.attr_desired_access = COM_API_GENERIC_RW;
...
PortInfo QueryPortInfo(const Port& port, QueryInfoType infoType) {
  ...
  if (infoType == QueryInfoType::FULLY) {
    ...
    if (GetCommTimeouts(tmpPort.hCom, &timeouts)) {
      portInfo._fully_timeouts_readIntervalTimeout =
          timeouts.ReadIntervalTimeout;
      portInfo._fully_timeouts_readTotalTimeoutMultiplier =
          timeouts.ReadTotalTimeoutMultiplier;
      portInfo._fully_timeouts_readTotalTimeoutConstant =
          timeouts.ReadTotalTimeoutConstant;
      portInfo._fully_timeouts_writeTotalTimeoutMultiplier =
          timeouts.WriteTotalTimeoutMultiplier;
      portInfo._fully_timeouts_writeTotalTimeoutConstant =
